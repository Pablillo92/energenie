    ...
    return {
        ...
        link: function(scope, element, attrs, parent) {
            ...
            var init = function () {
                ...
                var tabItems = '<li ng-repeat="(key, value) in tabs track by $index" ng-click="onTabTabbed($event, {{key}})" class="slider-slide-tab" ng-style="{\'color\':'+attrs.color+',\'min-width\':(tabs.length<3)?100/tabs.length+\'%\':\'\'}" ng-bind-html="value"></li>';
            ...
            var addEvents = function() {
                ...
                ionic.onGesture("swipeleft", scope.onSlideMove ,slider[0]);
                ionic.onGesture("swiperight", scope.onSlideMove ,slider[0]);
                ionic.onGesture("release", scope.onSlideChange ,slider[0]);
                scope.$watch(attrs.activeSlide,function(n,o){
                    if(n){
                        ionicSlideBoxDelegate.slide(n);
                        slideToCurrentPosition();
                    }
                });
            ...
            scope.addTabContent = function ($content,index) {
                var pos = scope.tabs.length;
                if(typeof(index)!="undefined") scope.tabs[index] = $content;
                else scope.tabs.push($content);
                scope.$apply();
            ...
            scope.editTabLabel = function ($content,id){
                scope.tabs[id] = $content;
        ...
        controller: ['$scope','$q',function($scope,$q) {
            this.addTab = function($content,index) {
                var deferred = $q.defer();
                $timeout(function() {
                    if($scope.addTabContent) {
                        var pos = $scope.addTabContent($content,index);
                        deferred.resolve(pos);
                    }else{ deferred.reject();}
                });
                return deferred.promise;
            };
            this.editTab = function($content,pos){
                var deferred = $q.defer();
                if($scope.editTabLabel) {
                    $scope.editTabLabel($content,pos);
                    deferred.resolve();
                }else{ deferred.reject();}
                return deferred.promise;
            };
...
slidingTabsDirective.directive('ionSlideTabLabel', [ function() {
    return {
        ...
        link: function ($scope, $element, $attrs, $parent) {
            var pos = 0,index=0;
            if(typeof($scope.pos)=="string") pos = parseInt($scope.pos);
            else if(typeof($scope.pos)!="undefined") pos = $scope.pos;
            

            $parent.addTab($scope.ionSlideTabLabel,pos).then(function(){
                $scope.$watch('ionSlideTabLabel',function(n){
                    if(n){
                        $parent.editTab(n,pos);
                    }
                });
            });
